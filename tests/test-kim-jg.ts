...
const raw = fs.readFileSync('public/test김재관.txt', 'utf-8');
const parts = raw.split(/^#.*$/m).filter(s => s.trim());
const expert = JSON.parse(parts[1].trim());

// 전문가 info: 1976.01.27 辰時(08:30) 양력 남
const code = calculateSaju({
  name: '김재관', gender: '남',
  birthYear: 1976, birthMonth: 1, birthDay: 27,
  birthHour: 8, birthMinute: 30,
  isLunar: false,
});
...
console.log(`${status} 김재관: ${pct}% (${ok}/${total})${fail>0?' — '+issues.join(', '):''}`);
