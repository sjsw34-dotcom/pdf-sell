...
const result = calculateSaju({
  name: '남성고객', gender: '남',
  birthYear: 1983, birthMonth: 10, birthDay: 15,
  birthHour: 3, birthMinute: 15, isLunar: true,
});

fs.writeFileSync('tests/result-male-code.json', JSON.stringify(result, null, 2), 'utf-8');
...
function compare(section: string, label: string, codeAux: string[], expertAux: string[]) {
  const codeSet = new Set(codeAux); const expertSet = new Set(expertAux);
...
console.log('═══ 사주팔자 기본 ═══');
const ePillar = [['편인','己','丑'],['일간(나)','辛','亥'],['식신','癸','亥'],['식신','癸','亥']];
